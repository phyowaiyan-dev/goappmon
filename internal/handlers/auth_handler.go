...
func (h *AuthHandler) LoginPage(c *gin.Context) {
	...
	_ = h.renderer.RenderPage(c.Writer, "login.html", PageData{Title: "Admin Login"})
}

func (h *AuthHandler) Login(c *gin.Context) {
	if err := c.Request.ParseForm(); err != nil {
		_ = h.renderer.RenderPage(c.Writer, "login.html", PageData{Title: "Admin Login", Error: "Invalid login form"})
		return
	}
	...
	if err := utils.ValidateEmail(email); err != nil {
		_ = h.renderer.RenderPage(c.Writer, "login.html", PageData{Title: "Admin Login", Error: err.Error(), LoginEmail: email})
		return
	}
	admin, err := h.authService.Authenticate(c.Request.Context(), email, password)
	if err != nil {
		_ = h.renderer.RenderPage(c.Writer, "login.html", PageData{Title: "Admin Login", Error: "Invalid email or password", LoginEmail: email})
		return
	}

	token, err := h.authService.SignSession(admin.ID)
	if err != nil {
		_ = h.renderer.RenderPage(c.Writer, "login.html", PageData{Title: "Admin Login", Error: "Failed to create session"})
		return
	}
